...
<head>
    ...
    <title>Keranjang Sepatu Bekas - Shopee Style</title>
...
<body class="bg-gradient-to-r from-sky-300 via-indigo-200 to-purple-300">
    ...
    <!-- Keranjang Section -->
    <section class="container mx-auto p-4 sm:p-6 mt-6">
        <h2 class="text-2xl sm:text-3xl font-semibold mb-4 text-theme-blue">Keranjang Belanja</h2>
        ...
        <div class="bg-white shadow-md rounded-lg p-4 sm:p-6">
            ...
            @if ($items->count() > 0)
                <table class="w-full text-left hidden md:table">
                    <thead>
                        <tr class="border-b">
                            <th class="py-3 text-gray-700">Produk</th>
                            <th class="py-3 text-gray-700 text-center">Harga</th>
                            <th class="py-3 text-gray-700 text-center">Jumlah</th>
                            <th class="py-3 text-gray-700 text-center">Total</th>
                            <th class="py-3 text-gray-700 text-center">Aksi</th>
                        </tr>
                    </thead>
                    <tbody id="cart-items">
                        @foreach ($items as $item)
                            <tr class="border-b" id="item-1">
                                <td class="py-4 flex items-center space-x-3">
                                    ...
                                    <div>
                                        <h3 class="text-gray-900 text-sm sm:text-base">{{ $item->name }}</h3>
                                    </div>
                                </td>
                                <td class="py-4 text-center text-sm">{{ $item->price }}</td>
                                <td class="py-4 text-center">
                                    <div class="flex items-center justify-center">
                                        <!-- Form untuk mengurangi kuantitas -->
                                        <form
                                            action="{{ route('decrease_quantity-keranjang', ['rowId' => $item->rowId]) }}"
                                            method="POST">
                                            @csrf
                                            @method('PUT')
                                            <button type="submit"
                                                class="border border-theme-blue text-theme-blue px-2 py-1">
                                                -
                                            </button>
                                        </form>

                                        <!-- Input untuk menampilkan kuantitas -->
                                        <input type="text" name="quantity" value="{{ $item->qty }}"
                                            min="1" class="w-10 sm:w-12 text-center border mx-2 p-1">

                                        <!-- Form untuk menambah kuantitas -->
                                        <form
                                            action="{{ route('increase_quantity-keranjang', ['rowId' => $item->rowId]) }}"
                                            method="POST">
                                            @csrf
                                            @method('PUT')
                                            <button type="submit"
                                ...
                                <td class="py-4 text-center text-sm" id="total-1">{{ $item->subtotal() }}</td>
                                <td class="py-4 text-center">
                                    <form action="{{ route('remove-keranjang', ['rowId' => $item->rowId]) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            class="text-red-500 hover:text-red-700 remove-cart">Hapus</button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
                ...
                <div
                    class="hidden md:flex flex-row justify-between items-center border-t pt-4 mt-6 bg-white p-4 rounded-lg shadow-lg">

                    <!-- Total Harga -->
                    <div class="text-lg sm:text-xl font-semibold">
                        <div class="text-lg sm:text-xl font-semibold flex items-center">
                            <span class="text-xl sm:text-2xl font-bold text-red-500" id="grand-total">
                                {{ Cart::instance('cart_' . Auth::user()->email)->subtotal() }}
                            </span>
                        </div>
                    </div>
                    ...
                    <a href="{{ route('checkout') }}"
                        class="bg-theme-blue text-white py-2 px-4 sm:px-6 rounded hover:bg-blue-700">
                        Checkout
                    </a>
                </div>
                ...
                <div class="block md:hidden space-y-6 px-4">
                    @foreach ($items as $item)
                        ...
                        <div class="border rounded-lg shadow-md p-4 bg-white">
                            <div class="flex flex-col sm:flex-row justify-between">
                                <!-- Informasi Kiri -->
                                <div class="flex space-x-4 w-full sm:w-3/4">
                                    <!-- Gambar Produk -->
                                    <img src="{{ asset('uploads/product/thumbnails') }}/{{ $item->model->image }}"
                                        alt="{{ $item->name }}"
                                    ...
                                    <div class="flex flex-col">
                                        <h3 class="text-gray-900 text-sm font-semibold truncate">{{ $item->name }}
                                        </h3>
                                ...
                                <!-- Jumlah dan Aksi -->
                                <div class="flex items-center justify-end mt-4 sm:mt-0 space-x-2">
                                    <form
                                        action="{{ route('decrease_quantity-keranjang', ['rowId' => $item->rowId]) }}"
                                        method="POST">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit"
                                            class="border border-theme-blue text-theme-blue px-3 py-1 rounded hover:bg-blue-100">
                                            -
                                        </button>
                                    </form>
                                    <input type="number" name="quantity" value="{{ $item->qty }}"
                                        min="1"
                                        class="w-12 text-center border mx-2 p-1 rounded focus:outline-none" readonly>
                                    <form
                                        action="{{ route('increase_quantity-keranjang', ['rowId' => $item->rowId]) }}"
                                        method="POST">
                                        @csrf
                            ...
                            <div class="flex justify-between items-center mt-4">
                                ...
                                <div class="text-sm font-semibold text-gray-700">
                                    Total: <span class="text-red-500">Rp
                                        {{ $item->subtotal() }}</span>
                                </div>
                    ...
                    <!-- Footer -->
                    <div
                        class="mt-8 flex flex-col sm:flex-row justify-between items-center border-t pt-4 space-y-4 sm:space-y-0">
                        <!-- Total Harga -->
                        <div class="text-lg sm:text-xl font-semibold">
                            <span class="text-xl sm:text-2xl font-bold text-red-500" id="grand-total">
                                {{ Cart::instance('cart_' . Auth::user()->email)->subtotal() }}
                        ...
                        <!-- Tombol Checkout -->
                        <a href="{{ route('checkout') }}"
                            class="bg-theme-blue text-white py-2 px-4 sm:px-6 rounded hover:bg-blue-700 shadow-md transform hover:scale-105 transition duration-300">
                            Checkout
                        </a>
                    </div>
                </div>
            @else
                <div class="flex flex-col items-center justify-center py-12 space-y-4">
                    <h2 class="text-xl sm:text-2xl font-semibold text-gray-700">Keranjang Belanja Kosong</h2>
                    <p class="text-gray-500 text-center sm:text-base text-sm">
                        Oops! Belum ada produk yang ditambahkan ke keranjang Anda.
                    </p>
                    <a href="{{ route('shop.category', ['category_id' => 0]) }}"
                        class="bg-theme-blue text-white py-2 px-6 sm:px-8 rounded hover:bg-blue-700 transition">
                        Mulai Belanja
                    </a>
                </div>
            @endif
    ...
    <script>
        ...
        document.querySelectorAll('.remove-cart').forEach(function(btn) {
            btn.addEventListener('click', function(e) {
                e.preventDefault(); // Mencegah perilaku default dari <a> tag
                this.closest('form').submit(); // Kirim form
            });
        });
    </script>
