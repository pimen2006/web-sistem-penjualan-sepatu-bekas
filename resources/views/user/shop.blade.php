...
<body class="bg-gradient-to-r from-sky-300 via-indigo-200 to-purple-300">
    ...
    <section class="bg-blue-800 text-white lg:px-5">
        <div class="container mx-auto flex flex-col lg:flex-row items-center py-12">
            <!-- Text Section -->
            <div class="lg:w-1/2 px-6 text-center lg:text-left">
                <h2 class="text-4xl font-bold mb-4">{{ $category ? $category->name : 'Second step' }}</h2>
                <p class="text-lg">{{ $category ? $category->deskripsi : 'menyediakan berbagai sepatu bekas yang berkualitas' }}</p>

            </div>
            <!-- Image Section -->
            <div class="lg:w-3/4 w-full mt-7 lg:mt-0">
                <img src="{{ $category ? asset('uploads/category/' . $category->image) : asset('img/sepatu1.jpg') }}"
                    alt="{{ $category ? $category->name : 'Default Category' }}"
                    class="w-full rounded-lg shadow-lg lg:h-[500px] object-cover h-60">
            </div>
        </div>
    </section>

    <main class="bg-gradient-to-r from-sky-300 via-indigo-200 to-purple-300">
        <div class="px-6 md:px-8 lg:px-10">
            {{-- search and category button --}}
            <div class="flex items-center justify-between gap-4 mt-8">
                <!-- Button Category -->
                <a href="{{ route('home') }}#pilihcategory"
                    class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 md:px-6 rounded-lg shadow-md text-sm md:text-base w-full md:w-auto text-center block">
                    View Categories
                </a>

                <!-- Search Form -->
                <form action="{{ route('shop.search', ['category_id' => $category ? $category->id : 0]) }}"
                    method="GET" class="max-w-md w-full">
                    ...
                    <div class="relative">
                        ...
                        <input type="search" id="default-search" name="search" value="{{ request('search') }}"
                            ...
                            placeholder="{{ $category ? 'Cari produk di kategori ' . $category->name : 'Cari produk' }}"
                            required />
                        <button type="submit"
            ...
            <div class="container mx-auto  py-4">
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-y-6 gap-x-8 py-3">
                    @foreach ($products as $product)
                        <a href="{{ route('detail_product', ['product_slug' => $product->slug]) }}"
                            ...
                            <img class="w-full lg:h-56 h-36 object-cover"
