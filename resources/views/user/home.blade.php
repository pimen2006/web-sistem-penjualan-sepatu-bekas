        ...
        <div class="bg-white shadow-md p-5">
            <div class="bg-white shadow-md p-5">
                <div class="relative flex">
                    <img class="w-full h-[300px] md:h-[400px] object-cover rounded-lg shadow-md"
                        src="{{ asset('img/sepatu1.jpg') }}" alt="Image 1">
                    <div
                        class="absolute inset-0 bg-gradient-to-t from-black/60 via-black/30 to-transparent flex items-center justify-center p-4 md:p-6 rounded-lg text-center">
                        <div class="text-white">
                            <h2 class="text-xl md:text-3xl font-bold mb-2">Second Stp</h2>
                            <p class="text-sm md:text-lg font-medium">
                                Second step ini menjual berbagai sepatu bekas yang terjangkau, berkualitas, dan stylis
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <section class="mt-6 grid grid-cols-3 gap-4">
                <div class="bg-gray-100 rounded-lg flex items-center gap-3 p-3 text-center flex-col">
                    <img src="{{ asset('img/1.png') }}" alt="Terjangkau"
                        class="lg:w-20 lg:h-16 w-12 h-12 object-contain">
                    <h3 class="lg:text-lg text-sm font-semibold text-gray-800">Terjangkau</h3>
                </div>
                <div class="bg-gray-100 rounded-lg flex items-center gap-3 p-3 text-center flex-col">
                    <img src="{{ asset('img/2.png') }}" alt="Berkualitas"
                        class="lg:w-16 lg:h-16 w-12 h-12 object-contain">
                    <h3 class="lg:text-lg text-sm font-semibold text-gray-800">Berkualitas</h3>
                </div>
                <div class="bg-gray-100 rounded-lg flex items-center gap-3 p-3 text-center flex-col">
                    <img src="{{ asset('img/3.png') }}" alt="Stylish"
                        class="lg:w-16 lg:h-16 w-12 h-12 object-contain">
                    <h3 class="lg:text-lg text-sm font-semibold text-gray-800">Stylish</h3>
                </div>
            </section>

            <hr id="pilihcategory" class="border-t border-gray-300 my-4">

            {{-- category --}}
            <div>
                <h1 class="text-2xl md:text-2xl font-bold text-gray-800 text-center mt-5 mb-4">
                    Category
                </h1>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-3 gap-4 pb-8 py-3 max-w-4xl mx-auto px-4">
                        @if ($categories->isEmpty())
                            <p class="text-center text-xl font-semibold text-gray-700 col-span-3">Tidak tersedia
                                kategori</p>
                        @else
                            @foreach ($categories as $category)
                                <a href="{{ route('shop.category', $category->id) }}"
                                    class="group block bg-white rounded-md shadow-md overflow-hidden transform transition hover:scale-105 hover:shadow-lg">
                                    <img src="{{ asset('uploads/category') }}/{{ $category->image }}"
                                        alt="{{ $category->name }}"
                                        class="w-full h-40 object-cover transition-transform duration-300 group-hover:scale-105" />
                                    <div class="p-2">
                                        <h2
                                            class="text-sm font-semibold mb-1 text-gray-800 group-hover:text-indigo-600">
                                            <input type="checkbox" class="hidden" name="categories"
                                                value="{{ $category->id }}">
                                            {{ $category->name }}
                                        </h2>
                                        <p class="text-xs text-gray-600 mb-2 truncate md:whitespace-normal">
                                            {{ $category->deskripsi }}</p>
                                        <span class="text-indigo-600 group-hover:underline text-xs">View collection
                                            &rarr;</span>
                                    </div>
                                </a>
                            @endforeach
            ...
            <div class="relative my-6">
                ...
                <span
                    class="absolute left-1/2 transform -translate-x-1/2 bg-white px-3 text-gray-800 font-bold text-lg md:text-2xl">
                    Top Shoes
                </span>
                <hr class="border-t border-gray-500 mt-5">
            </div>

            {{-- top shoes --}}
            <div class="container mx-auto p-4">
                <div class="grid grid-cols-2 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-y-6 gap-x-4 py-3">
                    @foreach ($products as $product)
                        <a href="{{ route('detail_product', ['product_slug' => $product->slug]) }}"
                            class="w-full md:w-auto max-w-xs rounded overflow-hidden shadow-lg bg-gray-100 transition-transform transform hover:scale-105 mb-6">
                            <img class="w-full lg:h-48 h-36 object-cover"
                                src="{{ asset('uploads/product') }}/{{ $product->image }}"
                                alt="{{ $product->name }}">
                            <div class="px-4 py-3">
                                <div class="font-bold text-lg mb-1">{{ $product->name }}</div>
                                <p class="text-gray-700 text-sm truncate">{{ $product->short_description }}</p>
                            </div>
