...
<body class="bg-gradient-to-r from-sky-300 via-indigo-200 to-purple-300">
    ...
    <div class="max-w-7xl mx-auto mt-12 grid grid-cols-1 lg:grid-cols-2 gap-12 px-6">
        <div class="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 overflow-hidden">
            <img class="w-full lg:h-96 h-72 object-cover" src="{{ asset('uploads/product') }}/{{ $product->image }}"
                alt="{{ $product->name }}">
            <div class="p-6">
                <h1 class="text-3xl font-extrabold text-gray-800">{{ $product->name }}</h1>
                <p class="text-gray-500 text-sm mt-2">{{ $product->short_description }}</p>
                <div class="text-gray-600 mt-4 space-y-2 leading-relaxed">
                    <p class="text-lg"><strong>Harga:</strong>
                        @if ($product->sale_price)
                            <span class="text-gray-500 line-through">${{ $product->regular_price }}</span>
                            <span class="text-blue-600 font-bold">${{ $product->sale_price }}</span>
                        @else
                            <span class="text-green-600 font-bold">${{ $product->regular_price }}</span>
                        @endif
                    </p>
                    <h3 class="text-xl font-semibold mt-6">Deskripsi:</h3>
                    <p class="text-gray-600 mt-2">{{ $product->description }}</p>
                    <h3 class="text-xl font-semibold mt-6">Stock:</h3>
                    <p class="text-gray-600 mt-2">{{ $product->quantity }}</p>
                    <div class="mt-4">
                        <span class="block font-medium text-gray-700">SKU:</span>
        ...
        <!-- Harga, Tombol, dan Detail Pengiriman -->
        <div class="bg-white rounded-lg shadow-md p-8 hover:shadow-lg transition-shadow duration-300">
            <div class="flex items-center justify-between">
                <span class="text-4xl font-bold text-gray-900">
                    @if ($product->sale_price)
                        <s class="text-gray-500">${{ $product->regular_price }}</s>
                        <span class="text-blue-600">${{ $product->sale_price }}</span>
                    @else
                        <span class="text-green-600">${{ $product->regular_price }}</span>
                    @endif
                </span>
            </div>

            <div class="mt-6 space-y-4 sm:space-y-0 sm:flex sm:space-x-4">
                @if (Cart::instance('cart_' . Auth::user()->email)->content()->where('id', $product->id)->count() > 0)
                    <a href="{{ route('keranjang') }}"
                        class="bg-gray-100 text-gray-800 px-6 py-3 rounded-lg font-semibold hover:bg-gray-200 transition duration-200 ease-in-out">
                        Pergi ke Keranjang
                    </a>
                @else
                    <form action="{{ route('add-keranjang') }}" method="POST"
                        class="w-full sm:w-auto flex items-center space-x-4">
                        @csrf
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            <input type="number" name="quantity" id="quantity" value="1" min="1"
                                ...
                                class="w-16 h-10 text-center border rounded-lg focus:outline-none focus:ring focus:ring-blue-300">
                        </div>
            ...
            <div class="bg-gray-50 rounded-lg p-4 mt-8 text-sm text-gray-600">
                <p>Dengan membeli sepatu bekas ini, kamu juga ikut berkontribusi dalam mengurangi limbah fashion dan
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 grid-rows-2 gap-4 sm:gap-6">
            @foreach ($products as $product)
                <a href="{{ route('detail_product', ['product_slug' => $product->slug]) }}"
                    class="w-full max-w-xs rounded overflow-hidden shadow-lg bg-gray-100 transition-transform transform hover:scale-105 mb-4 sm:mb-6 mx-auto">
                    <img class="w-full h-40 sm:h-44 md:h-52 object-cover"
                        src="{{ asset('uploads/product') }}/{{ $product->image }}" alt="{{ $product->name }}">
                    <div class="px-4 py-3">
                        <div class="font-bold text-base sm:text-lg mb-1">{{ $product->name }}</div>
                        <p class="text-gray-700 text-sm truncate">Deskripsi singkat tentang produk 1.</p>
                    </div>
